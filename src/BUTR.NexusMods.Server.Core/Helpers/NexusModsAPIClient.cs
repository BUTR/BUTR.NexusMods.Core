using BUTR.NexusMods.Server.Core.Models.NexusModsAPI;
...
namespace BUTR.NexusMods.Server.Core.Helpers
{
            ...
            try
            {
                ...
                if (await _cache.GetStringAsync(key) is { } modJson)
                {
                    return string.IsNullOrEmpty(modJson)
                        ? null
                        : JsonSerializer.Deserialize<NexusModsModInfoResponse>(modJson, _jsonSerializerOptions);
                }
